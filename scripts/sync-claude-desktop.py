...
def get_claude_desktop_config_path() -> Path | None:
    ...
    if "microsoft" in platform.uname().release.lower():
        try:
            ...
            return None
        return Path(
            f"/mnt/c/Users/{win_user}/AppData/Roaming/Claude/claude_desktop_config.json"
        )
...
def main():
    ...
    if not BEEMINDER_ENV.is_file():
        print(
            "[env] beeminder-utils not found at ~/code/beeminder-utils, skipping MCP sync"
        )
        return
    ...
    def replace_placeholders(obj):
        if isinstance(obj, str):
            return (
                obj.replace("__BEEMINDER_MCP_SERVER__", mcp_server_path)
                .replace("__BEEMINDER_USERNAME__", env.get("BEEMINDER_USERNAME", ""))
                .replace(
                    "__BEEMINDER_AUTH_TOKEN__", env.get("BEEMINDER_AUTH_TOKEN", "")
                )
            )
    ...
    if dry_run:
        # Redact resolved secrets: dry-run output lands in terminals and logs
        token = env.get("BEEMINDER_AUTH_TOKEN", "")
        rendered = json.dumps(config["mcpServers"], indent=2)
        if token:
            rendered = rendered.replace(token, "<redacted>")
        print("[env] [DRY_RUN] Would write:", rendered)
    else:
