...
# Homebrew locally; boxy/init.sh clones it under ~/.local/share on remotes
# (nothing there can install to /usr/local without root).
_antidote_dir=""
if type brew &>/dev/null; then
  _antidote_dir="$(brew --prefix)/opt/antidote/share/antidote"
elif [[ -d "$HOME/.local/share/antidote" ]]; then
...
# The -x test matters: this file is synced from my laptop, so it can land on a
# box whose dotfiles checkout hasn't been made yet.
if [[ -n "$NOTION_BOXY_NAME" && -z "$TMUX" && -t 0 && -x ~/code/dotfiles/scripts/tmux-dev.sh ]]; then
